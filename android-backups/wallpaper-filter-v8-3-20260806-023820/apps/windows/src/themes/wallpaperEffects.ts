...
export function applyWallpaperEffects(
    ...
    for (const target of effectTargets()) {
        ...
        target.dataset.neoWallpaperBrightness =
            normalized.backgroundBrightness.toFixed(2);

        target.dataset.neoWallpaperBlur =
            normalized.backgroundBlur.toFixed(0);

        target.dataset.neoContainerOpacity =
            normalized.containerOpacity.toFixed(2);
    }

    console.info(
        `[NeoWallpaperEffectsV82] apply brightness=${normalized.backgroundBrightness.toFixed(
            2
        )} blur=${normalized.backgroundBlur.toFixed(
            0
        )} opacity=${normalized.containerOpacity.toFixed(2)}`
    );
}
...
export function saveWallpaperEffects(
    ...
    if (typeof window !== "undefined") {
        try {
            ...
            // Live application remains available without storage.
        }
    }
...
function reapplyAfterThemeChange(): void {
    if (
    ...
    appearanceFrame =
        window.requestAnimationFrame(() => {
            ...
            const nextSurface =
                readThemeSurfaceRgb().join(",");

            if (nextSurface !== lastSurfaceKey) {
...
export function installWallpaperEffects(): void {
    ...
    console.info(
        "[NeoWallpaperEffectsV82] installed"
    );
}
