...
Neo Calendar's goal is to give users a robust and feature-ful calendar view into their Obsidian Vault. In addition to displaying and modifying events stored in note frontmatter and daily note bulleted lists, it can also read events from the Internet in CalDAV and ICS format.

Neo Calendar renders a custom React-based UI inspired by Notion Calendar. There is no calendar rendering library: every view is our own.
...
Much of the code of Neo Calendar exists to deal with the normalization of these formats so they can be handled by the view layer without worrying about what sources different events are actually from.
...
┌────────────┐   │              │       ┌─────────────┐     ┌──────────────┐
│            ├───►              ├───────►             ├─────►              │
│ EventStore │   │  EventCache  │       │ view.ts +   │     │  React UI    │
│            ◄───┤              ◄───────┤ calendar/   ◄─────┤ Components   │
└────────────┘   │              │       │ CalendarApp │     │              │
                 │              │       └─────────────┘     └──────────────┘
             ┌──►└──────▲──┬────┘
...
This module defines some common types used throughout the code. The most prevalent is `NeoEvent`, the intermediate representation every event in the plugin is normalized to, whatever source it came from. Its on-disk shape is specified in [`docs/event-format-spec.md`](../docs/event-format-spec.md).

`NeoEvent` is derived from a [Zod parser](https://github.com/colinhacks/zod) that handles parsing/validating JavaScript objects into the expected shape of an event. You can check out the parser in `types/schema.ts`.

Translation between `NeoEvent` and `DisplayEvent` (the view-layer type) is handled in `CalendarUtils.ts`. Each `Calendar` subclass (see below) handles its own translation from its source format into `NeoEvent`.
...
While `core` and `calendars` make up the Model in the `MVC` pattern, the Views and Controllers are currently both living in the `ui` directory. The React-based calendar UI lives in `calendar/CalendarApp.tsx`, which is mounted inside the Obsidian `ItemView` in `view.ts`. Most of the controller logic that interfaces with the `EventCache` lives in `view.ts`. Auxilliary views, like the edit/create modal and settings selectors, are React components that live in their own `.tsx` files and are mounted into the DOM when needed.
