...
function fakeWindow() {
    return {
        ...
        onResized: jest.fn(async (handler: () => void) => {
            resized = handler;
            return unlistenResize;
        }),
        onFocusChanged: jest.fn(async (handler: typeof focused) => {
            focused = handler;
...
test.each(["chargement", "accueil", "calendrier"])(
    "controls survive %s children and expose a reactive portal host",
    async (child) => {
        const window = fakeWindow();
        function Portal() {
            const slot = useDesktopTitlebarHost();
            return slot
                ? ReactDOM.createPortal(<span>portail</span>, slot)
                : null;
        }
        await act(async () => {
            ReactDOM.render(
                <DesktopWindowShell
                    actions={createDesktopWindowActions(window)}
                >
                    <main>{child}</main>
                    <Portal />
                </DesktopWindowShell>,
        ...
        expect(
            host.querySelector("#nc-desktop-titlebar-slot")!.textContent
        ).toContain("portail");
        await click("Réduire");
        await click("Agrandir");
        await click("Fermer");
...
test("window controls use colourable masks except for the restored window state", async () => {
    ...
    await act(async () => {
        ReactDOM.render(
            <DesktopWindowShell actions={createDesktopWindowActions(window)}>
                content
            </DesktopWindowShell>,
            host
        );
    ...
    const controls = Array.from(
        host.querySelectorAll(".nc-desktop-window-control")
    );
    expect(
        controls.map((control) => control.getAttribute("aria-label"))
    ).toEqual(["Réduire", "Agrandir", "Fermer"]);
    ...
    expect(
        host.querySelector('button[aria-label="Restaurer"] svg')
    ).not.toBeNull();
});
test("window controls use the supplied toolbar assets", async () => {
    ...
    expect(
        host
            .querySelector('button[aria-label="Réduire"] .nc-toolbar-icon')
            ?.getAttribute("data-toolbar-icon")
    ).toBe("minimize");
    expect(
        host
            .querySelector('button[aria-label="Agrandir"] .nc-toolbar-icon')
            ?.getAttribute("data-toolbar-icon")
    ).toBe("maximize");
    expect(
        host
            .querySelector('button[aria-label="Fermer"] .nc-toolbar-icon')
            ?.getAttribute("data-toolbar-icon")
    ).toBe("close");
});
test("calendar render failure leaves window controls usable", async () => {
    ...
    function Broken(): JSX.Element {
        throw new Error("calendar crash");
    }
...
test("late listener registration is cleaned after unmount", async () => {
    ...
    window.onResized.mockReturnValue(
        new Promise((resolve) => {
            resolveResize = resolve;
        })
    );
    window.onFocusChanged.mockReturnValue(
        new Promise((resolve) => {
